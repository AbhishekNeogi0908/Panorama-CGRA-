...
def main(app_name, C1_init_, C2_init_, cgra_cluster_r, cgra_cluster_c):
    
    # Setup paths
    input_path = f"../../../src/panorama_with_morpher/Morpher_DFG_Generator/benchmarks/morpher_benchmarks/{app_name}/cdfg.gexf"
    # Absolute output path in your home directory
    output_base = os.path.expanduser("~/panorama/data/results")
    output_path = os.path.join(output_base, f"Cluster_Mapping_{app_name}")
    
   # 2. PRINT LOADING LINE AT THE START (Not the end)
    print(f"Loading cdfg.gexf file from: {input_path}\n")

    if not os.path.exists(output_path):
        os.makedirs(output_path, exist_ok=True)

    # 2. REDIRECT OUTPUT (TERMINAL + FILE)
    log_file_path = os.path.join(output_path, "mapping_terminal_log.txt")
    ...
    try:
        #load cdfg/gexf file
        print(f"Loading cdfg.gexf file from: {output_path}")
       ...
       # --- RESTORED TERMINAL + AUTOMATED OUTPUT ---
        print("\nDFG cluster to CGRA cluster Mapping:")
        print("Cluster ID\t[Rows]\t[Cols]")
        
        # Define the file path for machine-readable results
        result_file = os.path.join(output_path, "cluster_mapping_result.txt")
        
        with open(result_file, "w") as f:
            for i in range(0, num_cluster_nodes):
                # DATA STRING: Constructing the cluster-to-PE-cluster coordinate string
                out_line = str(i) + '\t' + str(dfg_to_cgra_cluster_mapping_outcome_r[i]) + '\t' + str(dfg_to_cgra_cluster_mapping_outcome_c[i])
                
                # 1. PRINT to terminal: Restores your live view of the mapping coordinates
                print(out_line)
                
                # 2. WRITE to file: Saves data for downstream Morpher/PANORAMA tools
                f.write(out_line + '\n')

        print(f"\nSaving results to directory...")
        
        # 3. SAVE matrix-style cluster_mapping.txt (Grid view)
        mapping_file = os.path.join(output_path, "cluster_mapping.txt")
        with open(mapping_file, "w") as f:
            ...
            # Append node counts for each cluster to the bottom of the grid file
            i = 0
            for nodes in clus_dfg.nodes:
                f.write(str(i) + '\t' + str(clus_dfg.nodes[nodes]["label"]) + '\n')
                i = i + 1

        print(f"Mapping complete. All results saved to: {output_path}\n")
        ...
        # Ensures the session log file is correctly closed and flushed to disk
...
if __name__ == '__main__':
    if len(sys.argv) < 6:
        print("Expected args: python cluster_mapping.py <AppName> <C1> <C2> <Rows> <Cols>")
    else:
        # Pass the 5th argument (AppName) to main
    ...
    C1_init = sys.argv[2]
    C2_init = sys.argv[3]
    cgra_cluster_r = sys.argv[4]
    cgra_cluster_c = sys.argv[5]
    main(app_name,C1_init, C2_init,cgra_cluster_r, cgra_cluster_c)
